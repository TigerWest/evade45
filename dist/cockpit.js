...
export function createCockpit(camera){
  ...
  const vehicle=new THREE.Group();camera.parent.add(vehicle);
  const views=Object.fromEntries(['foot','bike','armor'].map(mode=>{const g=new THREE.Group();(mode==='foot'?root:vehicle).add(g);return [mode,g]}));
  const random=seededRandom(412),textures=[];
  function surface(base,fabric=false){
  ...
  const mat=(color,roughness=.8,metalness=0,map=null)=>new THREE.MeshStandardMaterial({color,roughness,metalness,map});
  ...
  const rods=new Map(),rings=new Map();
  ...
  function box(parent,x,y,z,w,h,d,material){const m=mesh(parent,cube,material,x,y,z);m.scale.set(w,h,d);return m}
  function oval(parent,x,y,z,w,h,d,material){const m=mesh(parent,sphere,material,x,y,z);m.scale.set(w,h,d);return m}
  ...
  function ring(parent,x,y,z,r,t,material){const key=`${r}:${t}`;if(!rings.has(key))rings.set(key,new THREE.TorusGeometry(r,t,8,40));return mesh(parent,rings.get(key),material,x,y,z)}
  function cable(parent,points,r=.006,material=black){return mesh(parent,new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points.map(p=>new THREE.Vector3(...p))),16,r,5,false),material)}
  function bolt(parent,x,y,z,r=.009){const g=new THREE.Group();g.position.set(x,y,z);parent.add(g);rod(g,[0,0,-.005],[0,0,.005],r,steel);box(g,0,0,.0055,r*1.05,.002,.0015,black);return g}
  function plate(parent,x,y,z,w,h){box(parent,x,y,z,w,h,.018,black);for(const sx of [-1,1])for(const sy of [-1,1])bolt(parent,x+sx*(w/2-.02),y+sy*(h/2-.02),z+.014,.006)}
  function label(parent,text,x,y,z,w,h,color='#b6bba1'){
  ...
  function hand(parent,side,x,y,z,grip=false){
