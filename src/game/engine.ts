...
export const JUMP = { speed: 5.1, gravity: 13.5, cost: 18, cooldown: 0.95 };
export const MODES: Record<Mode, ModeConfig> = {
  foot: { speed: 4.9, boost: 8.2, response: 12, radius: 0.38, health: 1, eye: 1.68 },
  bike: { speed: 13, boost: 23, response: 1.8, radius: 0.85, health: 1, eye: 1.48 },
...
export function seededRandom(seed = 73): () => number {
  return () => {
...
export function circleHitsRect(x: number, z: number, r: number, b: Obstacle): boolean {
  const cx = clamp(x, b.x - b.w / 2, b.x + b.w / 2),
    cz = clamp(z, b.z - b.d / 2, b.z + b.d / 2);
...
function clearApproach(x: number, y: number, z: number, tx: number, ty: number, tz: number) {
  ...
  return !OBSTACLES.some((b) => {
    let enter = 0,
      leave = 1;
    const axes = [
      [x, tx - x, b.x - b.w / 2 - 0.28, b.x + b.w / 2 + 0.28],
      [y, ty - y, -0.28, b.h + 0.28],
      [z, tz - z, b.z - b.d / 2 - 0.28, b.z + b.d / 2 + 0.28],
    ];
...
function spawnDrone(g: GameState) {
  const p = g.player,
    ...
    first = g.nextId === 1;
  const angle = first ? p.yaw + 0.16 : p.yaw + (g.random() - 0.5) * Math.PI * 2;
  const range = first ? 76 : 70 + g.random() * 24;
  const y = 5 + g.random() * 4,
    ty = p.altitude + (g.mode === 'armor' ? 1.35 : 0.85);
  ...
  for (let i = 0; i < 48; i++) {
    ...
    const x = p.x - Math.sin(angle + offset) * range,
      z = p.z - Math.cos(angle + offset) * range;
    if (
      !clearApproach(x, y, z, p.x, ty, p.z) ||
      !clearApproach(x, y, z, p.x + p.vx * 0.25, ty, p.z + p.vz * 0.25)
    )
      continue;
    const dx = p.x - x,
      dy = ty - y,
...
function explode(g: GameState, d: DroneState, hitPlayer: boolean) {
  g.explosions.push({ x: d.x, y: Math.max(0.3, d.y), z: d.z, age: 0, seed: d.id });
  d.phase = 'dead';
  if (hitPlayer) hit(g, 'drone');
  else g.dodges++;
}
function steerDrone(
  d: DroneState,
  ...
  const speed = Math.hypot(d.vx, d.vy, d.vz),
    nextSpeed = Math.min(diff.speed, speed + diff.acceleration * dt);
  const desiredLength = Math.max(0.001, Math.hypot(tx - d.x, ty - d.y, tz - d.z));
  const ax = d.vx / speed,
    ay = d.vy / speed,
    az = d.vz / speed;
  const bx = (tx - d.x) / desiredLength,
    by = (ty - d.y) / desiredLength,
    bz = (tz - d.z) / desiredLength;
  ...
  if (angle < 1e-6) {
    ...
    const turn = Math.min(angle, maxTurn),
      projection = ax * bx + ay * by + az * bz;
    let px = bx - ax * projection,
      py = by - ay * projection,
      pz = bz - az * projection;
    ...
    nx = ax * Math.cos(turn) + (px / plen) * Math.sin(turn);
    ny = ay * Math.cos(turn) + (py / plen) * Math.sin(turn);
    nz = az * Math.cos(turn) + (pz / plen) * Math.sin(turn);
  }
  const oldYaw = Math.atan2(-d.vx, -d.vz),
    newYaw = Math.atan2(-nx, -nz);
  const bank = clamp(
    -Math.atan2((angleDifference(newYaw, oldYaw) * nextSpeed) / dt, 9.81),
    -0.95,
    0.95,
  );
...
function simulate(g: GameState, input: GameInput, dt: number) {
  const p = g.player,
    cfg = MODES[g.mode],
    diff = DIFFICULTIES[g.difficulty];
  g.elapsed = Math.min(DURATION, g.elapsed + dt);
  p.invulnerable = Math.max(0, p.invulnerable - dt);
  g.collision = Math.max(0, g.collision - dt * 2);
  p.jumpCooldown = Math.max(0, p.jumpCooldown - dt);
  p.landing = Math.max(0, p.landing - dt * 3);
  ...
  if (p.verticalSpeed !== 0 || p.altitude > 0) {
    p.altitude += p.verticalSpeed * dt - 0.5 * JUMP.gravity * dt * dt;
    p.verticalSpeed -= JUMP.gravity * dt;
    if (p.altitude <= 0) {
      p.altitude = 0;
      p.verticalSpeed = 0;
      p.landing = 0.22;
  ...
  p.boosting =
    !!input.boost && Math.abs(forward) + Math.abs(right) > 0.05 && !p.exhausted && p.energy > 0;
  p.energy = clamp(p.energy + (p.boosting ? -24 : 13) * dt, 0, 100);
  ...
  if (g.mode === 'foot') {
    ...
    const damping = 1 - Math.exp(-(p.altitude > 0 ? 5 : cfg.response) * dt);
    p.vx += (tx - p.vx) * damping;
    p.vz += (tz - p.vz) * damping;
    p.bodyYaw = p.yaw;
    p.dodgeX *= Math.exp(-dt * 3);
    p.dodgeZ *= Math.exp(-dt * 3);
  } else {
    const steer =
      -right *
      (g.mode === 'bike' ? 1.35 : 0.78) *
      clamp(Math.abs(p.speed) / 2, 0.12, 1) *
      dt *
      (p.speed < -0.1 ? -1 : 1);
    p.bodyYaw += steer;
    p.yaw += steer;
    const desired = forward * (forward < 0 ? topSpeed * 0.35 : topSpeed);
    p.speed += (desired - p.speed) * (1 - Math.exp(-cfg.response * dt));
    p.vx = -Math.sin(p.bodyYaw) * p.speed;
    p.vz = -Math.cos(p.bodyYaw) * p.speed;
  }
  const blocks = (b: Obstacle) => p.altitude < b.h;
  const nextX = clamp(p.x + p.vx * dt, -WORLD_LIMIT + cfg.radius, WORLD_LIMIT - cfg.radius);
  if (!OBSTACLES.some((b) => blocks(b) && circleHitsRect(nextX, p.z, cfg.radius, b))) p.x = nextX;
  else {
    p.vx = 0;
    g.collision = 0.4;
    if (g.mode !== 'foot') p.speed *= Math.exp(-dt * 20);
  }
  const nextZ = clamp(p.z + p.vz * dt, -WORLD_LIMIT + cfg.radius, WORLD_LIMIT - cfg.radius);
  if (!OBSTACLES.some((b) => blocks(b) && circleHitsRect(p.x, nextZ, cfg.radius, b))) p.z = nextZ;
  else {
    p.vz = 0;
    g.collision = 0.4;
    if (g.mode !== 'foot') p.speed *= Math.exp(-dt * 20);
  ...
  if (g.elapsed >= g.nextDrone) {
    ...
    g.nextDrone = blocked
      ? g.elapsed + 0.5
      : g.nextDrone + diff.interval * (1 - (g.elapsed / DURATION) * 0.25);
  }
  g.nearest = Infinity;
  g.threat = 0;
  for (const d of g.drones) {
    ...
    let dx = p.x - d.x,
      dz = p.z - d.z;
    const distance = Math.hypot(dx, p.y * 0.5 + p.altitude * 0.5 - d.y, dz);
    g.nearest = Math.min(g.nearest, distance);
    d.closest = Math.min(d.closest, distance);
    if (d.phase === 'approach') {
    ...
    const speed = Math.hypot(d.vx, d.vy, d.vz);
    g.peakDroneSpeed = Math.max(g.peakDroneSpeed, speed);
    const slices = Math.max(1, Math.ceil((speed * dt) / 0.12));
    for (let j = 0; j < slices && d.phase !== 'dead'; j++) {
      d.x += (d.vx * dt) / slices;
      d.y += (d.vy * dt) / slices;
      d.z += (d.vz * dt) / slices;
      const bodyY = clamp(d.y, p.altitude + 0.18, p.altitude + cfg.eye - 0.1);
      const close = Math.hypot(d.x - p.x, d.y - bodyY, d.z - p.z) < cfg.radius + 0.24;
      const obstacle = OBSTACLES.some(
        (b) => d.y < b.h + 0.12 && d.y > -0.12 && circleHitsRect(d.x, d.z, 0.22, b),
      );
      if (close) explode(g, d, true);
      else if (d.y < 0.12 || obstacle) explode(g, d, false);
    }
    dx = p.x - d.x;
    dz = p.z - d.z;
    ...
    if (d.phase === 'commit' && departing && !d.passed) {
      ...
      if (d.closest < 4) {
        ...
        g.lastPass = {
          time: g.elapsed,
          side: clamp((d.x - p.x) * Math.cos(p.yaw) - (d.z - p.z) * Math.sin(p.yaw), -1, 1),
          gap: d.closest,
          speed,
        };
      }
    }
    if (d.phase !== 'dead' && (d.age > 13 || (d.phase === 'pass' && Math.hypot(dx, dz) > 35))) {
      d.phase = 'dead';
      g.dodges++;
    }
    if (d.phase === 'commit') g.threat = 1;
    else if (d.phase === 'approach' && distance < 40) g.threat = Math.max(g.threat, 0.6);
  }
  g.drones = g.drones.filter((d) => d.phase !== 'dead');
  g.explosions = g.explosions.filter((e) => (e.age += dt) < 1.2);
...
export function stepGame(g: GameState, input: GameInput, delta: number) {
  ...
  while (remaining > 1e-9 && g.status === 'playing') {
    const dt = Math.min(remaining, PHYSICS_STEP, DURATION - g.elapsed);
    if (dt <= 1e-9) {
      g.elapsed = DURATION;
      g.status = 'won';
      break;
