  ...
  for (const locale of Object.keys(messages))
    for (const mode of Object.keys(MODES))
      for (const difficulty of Object.keys(DIFFICULTIES)) {
        const values = {
          mode: translate(locale, `mode.${mode}`),
          difficulty: translate(locale, `difficulty.${difficulty}`),
          time: '12.5',
        };
        for (const key of [
          'result.wonDescription',
          'result.lostDescription',
          'result.lostKicker',
        ]) {
          const result = translate(locale, key, values);
          assert.doesNotMatch(result, /\{\w+\}/);
          assert.ok(result.includes(values.mode));
        }
        assert.ok(
          translate(locale, 'spec.summary', { speed: 97, source: 'Avata 2 / Manual' }).includes(
            '97 km/h',
          ),
        );
      }
...
test('switching locale replaces content and accessibility labels without removing markup', () => {
  ...
  const title = element({ 'data-i18n': 'page.title' }),
    label = element({ 'data-i18n': 'start' }),
    button = element({ 'data-i18n-aria': 'help.openLabel' }),
    meta = element({ 'data-i18n-content': 'page.description' });
  const root = {
    documentElement: { lang: 'ko' },
    querySelectorAll(selector) {
      const attribute = selector.slice(1, -1);
      return [title, label, button, meta].filter((e) => attribute in e.attributes);
    },
  };
